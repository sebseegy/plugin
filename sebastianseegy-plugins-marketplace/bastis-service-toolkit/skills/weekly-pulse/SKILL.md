...
Write into Basti's local clone of the `sebastianseegy/Plugins` GitHub repo —
currently `/Users/sebastianse/Documents/GitHub/Plugins/sebastianseegy-plugins-marketplace/bastis-service-toolkit`
(this is the actual source the live plugin installs from via the
`sebastianseegy-plugins` marketplace, so edits here are what matters — not the
`skills copy` folder on the Desktop, which was a pre-marketplace workaround and
is no longer the canonical copy). Resolve the path by locating the connected
folder rather than assuming this exact string, since it may move.

After writing the changelogs, also bump the patch version in
`.claude-plugin/plugin.json` (the file is at the root of the
`bastis-service-toolkit/` directory in the Plugins repo). Read the current
`version` string, increment the patch number (e.g. `1.0.1` → `1.0.2`), and
write it back. This is what makes Claude Code's auto-update actually fire —
it compares version strings, so without a bump it always reports "already at
latest" even with new content.

Then run `git add -A && git commit -m "weekly-pulse: <date range> findings"` in
the repo directory. No push — there's no network path to GitHub from this
environment. End the digest with a one-line note when changelog writes and a
version bump happened, e.g. "Changelog updates written, version bumped to X.Y.Z
— ready to push." Skip the note on runs where nothing was written.
